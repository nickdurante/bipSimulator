...
void n_graphics_context_set_antialiased(n_GContext * ctx, char enable) {
    ctx->antialias = enable;
}

void n_graphics_context_set_stroke_width(n_GContext * ctx, unsigned short width) {
    if (width == 0)
        return;
    ctx->stroke_width = ((width + 2) & ~(1)) - 1;
}

void n_graphics_context_set_stroke_caps(n_GContext * ctx, char stroke_caps) {
...
char n_graphics_release_frame_buffer(n_GContext * ctx, GBitmap * bitmap) {
#ifndef NGFX_IS_CORE
...
n_GContext * n_graphics_context_from_buffer(char * buf) {
...
n_GContext * n_root_graphics_context_from_buffer(char * buf) {
    n_GContext * out = n_graphics_context_create();
    out->fbuf = buf;
    return out;
}
