...
static const char n_prv_palette_size[] = {
...
char n_gbitmapformat_get_bits_per_pixel(n_GBitmapFormat format) {
    static const char n_prv_bits_per_pixel[] = {
...
unsigned short n_gbitmap_get_bytes_per_row(const n_GBitmap *bitmap) {
...
char *n_gbitmap_get_data(const n_GBitmap *bitmap) {
    return bitmap->addr;
}

void n_gbitmap_set_data(n_GBitmap *bitmap, char *data, n_GBitmapFormat format, unsigned short row_size_bytes, char free_on_destroy) {
...
void n_gbitmap_set_palette(n_GBitmap *bitmap, n_GColor *palette, char free_on_destroy) {
...
n_GBitmap *n_gbitmap_create_with_data(const char *data) {
...
static n_GBitmap *n_prv_gbitmap_create_with_palette(n_GSize size, n_GBitmapFormat format, n_GColor *palette, char free_on_destroy) {
    n_GBitmap *bitmap = (n_GBitmap*)NGFX_PREFERRED_malloc(sizeof(n_GBitmap));
    if (bitmap == NULL) {
        return NULL;
    }
    
    char bits_per_pixel = n_gbitmapformat_get_bits_per_pixel(format);
    bitmap->row_size_bytes = (size.w * bits_per_pixel + 7) / 8;
    bitmap->addr = (char*)NGFX_PREFERRED_malloc(size.h * bitmap->row_size_bytes);
    if (bitmap->addr == NULL) {
...
static n_GBitmap *n_prv_gbitmap_create(n_GSize size, n_GBitmapFormat format) {
    char palette_size = n_prv_palette_size[format];
...
n_GBitmap *n_gbitmap_create_blank_with_palette(n_GSize size, n_GBitmapFormat format, n_GColor *palette, char free_on_destroy) {
...
n_GBitmap *n_gbitmap_create_palettized_from_1bit(const n_GBitmap *src_bitmap) {
    ...
    if (dst_bitmap->row_size_bytes == src_bitmap->row_size_bytes && src_bitmap->bounds.origin.x == 0) {
        ...
        const char *src_address = src_bitmap->addr + src_bitmap->bounds.origin.y * src_bitmap->row_size_bytes;
        ...
        char off = src_bitmap->bounds.origin.x % 8;
        short x, y;
        char *dst_pixel = dst_bitmap->addr;
        char *src_pixel_line = src_bitmap->addr + src_bitmap->bounds.origin.x / 8;
        src_pixel_line += src_bitmap->bounds.origin.y * src_bitmap->row_size_bytes;
        for (y = 0; y < dst_bitmap->bounds.size.h; y++) { 
            char *src_pixel = src_pixel_line;
            ...
            if (dst_bitmap->bounds.size.w % 8 != 0) {
                char bits_left = 8 + dst_bitmap->bounds.size.w - x;
