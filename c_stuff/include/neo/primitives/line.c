...
void n_graphics_prv_draw_1px_line_bounded(n_GContext * ctx,
    ...
    char iterate_over_y = false;
#ifdef PBL_BW
    char color = __ARGB_TO_INTERNAL(ctx->stroke_color.argb);
#endif
    short dy = (to.y - from.y), dx = (to.x - from.x);
    if (abs(dy) > abs(dx)) {
        iterate_over_y = true;
    }
    ...
    if (iterate_over_y) {
        ...
        short begin = __BOUND_NUM(miny, from.y, maxy - 1);
        short end = __BOUND_NUM(miny, to.y, maxy - 1);
        if (e == 0) {
#ifdef PBL_BW
            ...
            for (short y = begin; y <= end; y++) {
                short x = (dx * (y-from.y) * 2 + e * dy) / (dy * 2) + from.x;
                if (x < minx || x >= maxx) {
                    continue;
                }
#ifdef PBL_BW
        ...
        signed char e = (dy == 0 ? 0 : (dy > 0 ? 1 : -1));
        short begin = __BOUND_NUM(minx, from.x, maxx - 1);
        short end = __BOUND_NUM(minx, to.x, maxx - 1);
        if (e == 0) {
#ifdef PBL_BW
            ...
            for (short x = begin; x <= end; x++) {
                short y = (dy * (x-from.x) * 2 + e * dx) / (dx * 2) + from.y;
                if (y < miny || y >= maxy) {
                    continue;
                }
#ifdef PBL_BW
...
static unsigned short prv_fast_int_sqrt(unsigned short in) {
    unsigned short a = 1, b = in;
...
void n_graphics_prv_draw_thick_line_bounded(n_GContext * ctx,
                                               ...
                                               short minx, short maxx,
                                               short miny, short maxy) {
    unsigned short radius = (width - 1) / 2;
    if (ctx->stroke_caps) {
    ...
    short dx = to.x - from.x, dy = to.y - from.y;
    if (dx > 100) {
        dy = dy * 100 / dx;
        dx = 100;
    } else if (dy > 100) {
    ...
    signed char xdir = (from_a.x > from_b.x ? -1 : 1);
    signed char ydir = (from_a.y > from_b.y ? -1 : 1);
